...
xyplot(FSC.A ~ FSC.H, data=flowData, xlim=c(0,3e6), ylim=c(0,3e6), smooth=F, filter=pg.singlets, outline=T, displayFilter=TRUE, stat=T, pos=0.5, abs=T)
...
plot(flowData[[5]], c('FSC.A','FL1.A'), xlim=c(0,2e6), ylim=c(0,5e3), smooth=F)
Cgate <- locator(10)
gm.3 <- matrix(,10,2)
...
xyplot(FL1.A~FSC.A,data=flowData[[5]], xlim=c(0,5e6), ylim=c(0,5e4), filter=pg.nongfp, smooth=F, xbin=1024, stat=T, pos=0.5, abs=T)
...
plot(flowData[[1]], c('FSC.A','FL1.A'), xlim=c(0,5e6), ylim=c(0,5e5), smooth=F)
Dgate <- locator(10)
gm.4 <- matrix(,10,2)
...
xyplot(FL1.A~FSC.A,data=flowData[[1]], xlim=c(0,5e6), ylim=c(0,5e5), filter=pg.gfp, smooth=F, xbin=1024, stat=T, pos=0.5, abs=T)

#test that the fluorescing gate looks reasonable over all the samples
xyplot(FL1.A~FSC.A,data=flowData, xlim=c(0,5e6), ylim=c(0,5e5), filter=pg.gfp, smooth=F, xbin=1024, stat=T, pos=0.5, abs=T)
